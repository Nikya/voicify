...
class Player(object):
    ...
    def playlist_preview(self, item):
        """Play Item Immediately"""
        item = self.__quote(item)
        self.request("playlist preview %s" % (item))
        
    def playlist_preview_stop(self):
        """Play Item Immediately"""
        self.request("playlist preview cmd:stop")
        
    def playlist_save(self,item):
        """Sauver Playlist"""
        sitem = self.__quote(item)
        self.request("playlist save %s" % (item))
    ...
    def playlist_play_artist(self, item):
        """Play Item Immediately"""
        item = self.__quote(item)
        self.request("playlist loadalbum * %s *" % (item))
        
    def playlist_play_album(self, item):
        """Play Item Immediately"""
        item = self.__quote(item)
        self.request("playlist loadalbum * * %s" % (item))
        
    def playlist_play_genre(self, item):
        """Play Item Immediately"""
        item = self.__quote(item)
        self.request("playlist loadalbum %s * *" % (item))
    ...
    def playlist_add(self, item):
        ...
        item = self.__quote(item)
        self.request("playlistcontrol cmd:add playlist_name:%s" % (item))

    def playlist_insert(self, item):
        """Insert Item Into Playlist (After Current Track)"""
        item = self.__quote(item)
        self.request("playlistcontrol cmd:insert playlist_name:%s" % (item))
    ...
    def playlist_resume(self, playlist):
        ...
        return self.request("playlist resume %s" % (item))
